# job-tracker-ai-demo

![Next.js](https://img.shields.io/badge/Frontend-Next.js-black)
![FastAPI](https://img.shields.io/badge/Backend-FastAPI-green)
![PostgreSQL](https://img.shields.io/badge/Database-PostgreSQL-blue)
![Vercel](https://img.shields.io/badge/Deployed-Vercel-black)
![Render](https://img.shields.io/badge/API-Render-purple)

Full-stack job application tracking system with AI-assisted tools, built using Next.js, FastAPI, and PostgreSQL and deployed on Vercel and Render.

---

## Live Demo

Frontend: https://job-tracker-ai.vercel.app  
Backend API: https://job-tracker-ai-rryn.onrender.com  

---

## Features

- Track job applications with company, role, status, and notes
- Edit and update applications
- Dashboard with application summaries
- AI-powered career tools:
  - Resume Feedback Generator
  - Cover Letter Generator
  - Networking Message Generator
  - Interview Preparation Assistant
  - Follow-Up Email Generator
  - Job Match Score Analyzer
  - Application Insights Analyzer

---

## Tech Stack

**Frontend**

- Next.js
- TypeScript
- Tailwind CSS

**Backend**

- FastAPI
- Python
- SQLAlchemy

**Database**

- PostgreSQL (Neon)

**Deployment**

- Vercel (Frontend)
- Render (Backend)

---

## Screenshots

### Home Dashboard
![Home](home.png)

### Edit Application
![Edit Application](edit-application.png)

### Resume Feedback Tool
![Resume Feedback](resume-feedback.png)

### Cover Letter Generator
![Cover Letter](cover-letter.png)

### Networking Message Generator
![Networking Message](networking-message.png)

### Interview Preparation Tool
![Interview Prep](interview-prep.png)

### Follow-Up Email Generator
![Follow Up Email](follow-up-email.png)

### Job Match Analyzer
![Job Match](job-match.png)

### Application Insights Tool
![Application Insights](application-insights.png)

---

## Architecture

Frontend communicates with FastAPI backend via REST API.

Backend handles:

- CRUD operations
- AI tool logic
- Database persistence

---

## Purpose

This project was built to demonstrate full-stack development skills, including:

- Frontend UI/UX design
- REST API development
- Database integration
- Deployment to production
